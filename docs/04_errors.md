...
------------
```haskell
Illusions of control are common even in purely chance situations. They are particularly likely to occur in setting that are characterized by personal involvement, familiarity, foreknowledge of the desired outcome, and a focus on success.
```
Suzanne C. Thompson     
...
someOtherFun x = if (predicate on x) goodThing else throwM $ badThingException
