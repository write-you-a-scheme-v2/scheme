...
getVar (Atom atom) = do
  ...
  case Map.lookup atom env of
      Just x -> return x
      Nothing -> throwError $ Default  $ "error on getVar: " ++ (T.unpack atom)
...
eval (List [Atom fn, arg1, arg2]) = do
...
eval (List ((:) x xs)) = do
  ...
  case funVar of
      ...
      _                -> throwError $ NotFunction "tried to evaluate non-function: " (show funVar)
--
